...
class MotorControl:
    ...
    def __init__(self, channel:str, bitrate:int=8000000):
        """
        ...
        #    serial_device.close()
        #self.serial_.open()
        self.canbus = can.interface.Bus(channel=channel, interface='socketcan', bitrate=bitrate, local_loopback=False, fd=True)
        #print("can is open")
    ...
    def recv(self):
        ...
        data_recv = self.canbus.recv(0.01)
    ...
    def control_1c4(self, cur: np.uint8):
        data_buf = np.array([0x00, 0x00, 0x00, 0x00, cur, 0x00, 0x00, 0x00], np.uint8)
        addr = 0x4FE
        self.__send_data(addr, data_buf)
...
class DamiaoPort:
    def __init__(self, device, types, can_ids, master_ids, motor_with_torque, control_mode=Control_Type.MIT):
        ...
        self.control = MotorControl(self.device)
    ...
    def move_towards_sync2(self, goal_positions, kps, kds):
        for motor, goal_position, kp, kd in zip(self.motors, goal_positions, kps, kds):
            delta = goal_position - motor.getPosition()
            v = motor.getVelocity()
            tau = kp * delta - kd * v
            motor.goal_position = goal_position
            motor.goal_tau = tau
            self.control.controlMIT2(motor, 0, 0, 0, 0, tau)
        for motor in self.motors:
            self.control.recv()
    ...
    def control_1c4(self, cur: np.uint8):
        data_buf = np.array([0x00, 0x00, 0x00, 0x00, cur, 0x00, 0x00, 0x00], np.uint8)
        addr = 0X4FE
        self.control.__send_data(addr, data_buf)
